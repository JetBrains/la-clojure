 ...
 * @author ilyas
 */
public final class ClojureConsoleViewImpl extends JPanel implements ConsoleView, ObservableConsoleView, DataProvider, OccurenceNavigator {
  private static final Logger LOG = Logger.getInstance("#org.jetbrains.plugins.clojure.runner.console.ClojureConsoleViewImpl");
  ...
  public ClojureConsoleViewImpl(final Project project) {
  ...
  private Editor doCreateEditor() {
    ...
    final ClojureConsoleViewImpl consoleView = this;
  ...
  private class CopyAction extends AnAction{
    public CopyAction(){
      super(myEditor != null && myEditor.getSelectionModel().hasSelection() ? ClojureBundle.message("copy.selected.content.action.name") : ClojureBundle.message("copy.content.action.name"));
    }
  ...
  private static final DataAccessor<ClojureConsoleViewImpl> CONSOLE = new DataAccessor<ClojureConsoleViewImpl>() {
    public ClojureConsoleViewImpl getImpl(final DataContext dataContext) throws NoDataException {
      return DataAccessors.EDITOR.getNotNull(dataContext).getUserData(CONSOLE_VIEW_IN_EDITOR_VIEW);
    }
  };

  private static final Condition<ClojureConsoleViewImpl> CONSOLE_IS_RUNNING = new Condition<ClojureConsoleViewImpl>() {
    public boolean value(final ClojureConsoleViewImpl consoleView) {
      return consoleView.myState.isRunning();
    }
  };

  private static final DataAccessor<ClojureConsoleViewImpl> RUNNINT_CONSOLE = DataAccessor.createConditionalAccessor(CONSOLE, CONSOLE_IS_RUNNING);

  private abstract static class ConsoleAction extends AnAction {
    public void actionPerformed(final AnActionEvent e) {
      final DataContext context = e.getDataContext();
      final ClojureConsoleViewImpl console = RUNNINT_CONSOLE.from(context);
      execute(console, context);
    }

    protected abstract void execute(ClojureConsoleViewImpl console, final DataContext context);

    public void update(final AnActionEvent e) {
      final ClojureConsoleViewImpl console = RUNNINT_CONSOLE.from(e.getDataContext());
      e.getPresentation().setEnabled(console != null);
    }
  }

  private class EnterHandler extends ConsoleAction {
  ...
  public String getNextOccurenceActionName() {
    return ClojureBundle.message("down.the.stack.trace");
  }

  public String getPreviousOccurenceActionName() {
    return ClojureBundle.message("up.the.stack.trace");
  }
  ...
  private void deleteUserText(int offset, int length) {
    ClojureConsoleViewImpl consoleView = this;
    final Editor editor = consoleView.myEditor;
    final Document document = editor.getDocument();
    final int startOffset;
    final int endOffset;
